...
const TimerGame = ({ title, date, _id, format }) => {
  ...
  return (
    <motion.div className="bg-black p-2 m-2 rounded flex justify-between items-center">
      <motion.div
      ...
      >
        ...
        >
          ...
          {format === 'seconds'
            ? `${timeWeHave}`
            : `${Math.floor(timeWeHave / (3600 * 24))}:${Math.floor(
                (timeWeHave % (3600 * 24)) / 3600,
              )}:${Math.floor((timeWeHave % 3600) / 60)}:${Math.floor(
                timeWeHave % 60,
              )}`}
        </div>
      </motion.div>
      {location.pathname === '/' ? (
        ...
        <i
          className="pi pi-times text-gray-800"
          onClick={() => deleteT(_id)}
          aria-label="Filter"
        />
      )}
